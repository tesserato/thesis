...
fig.update_xaxes(showline=False, showgrid=False, zerolinewidth=1, zerolinecolor='gray', range=[164, 689])
...
ctr = np.argmax(np.abs(mult))

fig.add_trace(
  go.Scatter(
    name="First waveform", # <|<|<|<|<|<|<|<|<|<|<|<|
    x=np.arange(average_waveform.size) - r + ctr,
    y=average_waveform,
    # fill="toself",
    mode="lines",
    line=dict(
...
s = 10
start = ctr + average_waveform.size - r
avg = mode(T)
sd = np.sqrt(np.average((avg - T)**2))

print(avg, sd)

f = interpolate.interp1d(np.linspace(0, 1, average_waveform.size), average_waveform, "cubic")
mult = []

for i in range(int(avg - 3 * sd), int(avg + 3 * sd), s):
  print(i)
  mult.append(np.average(W[start : start + i] * f(np.linspace(0, 1, i))))
...
for i in range(int(avg - 3 * sd), int(avg + 3 * sd), s):
  val = mult[int((i - int(avg - 3 * sd)) / s)]**2
  color = f"rgba(0, 0, 255, {round(val, 4)})"
  wid = 1
  if val == 1.0:
    wid = 2
  print(val)
  fig.add_trace(
    go.Scatter(
      name=f"{i}", # <|<|<|<|<|<|<|<|<|<|<|<|
      x=np.arange(i) + start - 1,
      y=f(np.linspace(0, 1, i)),
      # fill="toself",
      mode="lines",
      line=dict(
          width=wid,
