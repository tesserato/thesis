...
fig.update_yaxes(showline=False, showgrid=False, zerolinewidth=1, zerolinecolor='black', range=[-.9, .9],title="<b>Amplitude</b>", row=1, col=1)
fig.update_yaxes(showline=False, showgrid=False, zerolinewidth=1, zerolinecolor='black', range=[-.09, .09], row=1, col=2)

fig.update_xaxes(showline=False, showgrid=False, zerolinewidth=1, zerolinecolor='black',title="<b>Frame</b>", row=1, col=1)
fig.update_xaxes(showline=False, showgrid=False, zerolinewidth=1, zerolinecolor='black',title="<b>Frame</b>", row=1, col=2)
...
l = average_waveform.size - 1
fig.add_trace(
  go.Scatter(
    name="Sinusoid    ", # <|<|<|<|<|<|<|<|<|<|<|<|
    # x=XX,
    y = np.max(np.abs(average_waveform)) * np.sin(2 * np.pi * np.arange(l + 1) / l),
    # fill="toself",
    mode="lines",
    line=dict(
        width=3,
