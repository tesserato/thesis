...
# # plot abs pulse avgs x pulse number
...
#   go.Scatter(
#     # x=X,
#     y=np.abs(Y_avg[1:]) / np.abs(Y_avg[:-1]),
#     name="Pulses",
#     mode="markers+lines",
#     marker=dict(
#         # size=8,
...
# FT = np.fft.rfft(np.abs(Y_avg) - np.average(np.abs(Y_avg)))
...
# # correlation = np.correlate(Y_avg, Y_avg, "full")
