...
ratios = np.zeros(Y_avg.size - 1)

for i in range(ratios.size):
  a = np.abs(Y_avg[i])
  b = np.abs(Y_avg[i + 1])
  ratios[i] = min(a, b) / max(a, b)

avgratios = np.average(ratios)
