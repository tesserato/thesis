...
idx = np.argmax(A)
# print(W)
print(f"ORIGINAL: f={round(F[idx], 2)} p={round(P[idx], 2)}")
#######################
...
for x in range(n):
  ...
  prob += pulp.LpConstraint(a + r, rhs=0 , sense=pulp.LpConstraintGE, name=f"c_{x}_neg") # a >= - r
  O.append(a)

prob += pulp.lpSum([O[x] * abs(W[x]) for x in range(len(O))]) # <|<|<|<|<|<|<|<|
