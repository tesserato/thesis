...
\begin{abstract} % 250 words
  % background
  TODO low res samples, interest in best sinusoidal approximation
  % motivation
  The Discrete Fourier Transform decomposes an periodic wave in a finite set of predefined sinusoids, commonly with frequencies coinciding with the natural numbers, offering no guarantees that one of those frequencies is the best one, in the sense of the frequency that generates the closest sinusoid to the original wave.
  % objective
  Starting from the observation of the intersection of plane waves in the frequency versus phase space we propose a mixed-integer programming formulation of the problem.
  % methods
  The algorithm is based on the representation of the set of samples in a frequency versus phase space and the observation of the interference pattern thus generated. Each sample defines a sinusoidal plane wave in this space, with a particular amplitude, direction and frequency. The determination of the point in which maximum constructive interference occurs translates to the determination of the closest point to the lines that represent the maxima of each plane wave.
  % results
  TODO: better accuracy than FFT, more time consuming
  % implications
...
\subsection{Theory}
  ...
  Of interest to this work are those sinusoids, as shown in Figure \ref{fig:OptSinusoids}, that have one of their infinite extrema, either maximum or minimum, coinciding with the abscissa $i$ or, setting $a=W[t]$, coincident with the point $(i, W[i])$; those sinusoids represent the combinations of phase an frequency that would maximize the value of $W[t] S[t]$ for frame $t$.
