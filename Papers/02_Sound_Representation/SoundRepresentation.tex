...
\begin{abstract}
We propose representing a discrete wave based on the description of the pseudo cycle as a function of time.
...
The idea, based on the Fourier transform theory, that a complex wave can be represented by a sum of sinusoids is ubiquituous in sound processing, owing considerably to the reversebility of the fourier transform.

Despite its many uses, for discrete real waves, frequency and time domain representations are equivalent, as the later represents the discrete with the same number of floats.

Additionally, eventual variations in the envelope of the wave have to be encoded as additional sinusoids \textcite{2019TarjanoNeuro}.

% Literature review | Brief Context of Prior Research
For these reasons, mixed representations were introduced, specially in the context of lossy and lossless sound compression

% Restate Your Question as Something Not Fully Understood by Prior Research -> identify the gap in the literature use but, however, etc…

However, those representations lack a more physical connection, and generally dont lend themselves well as basis representatios for further tasks, such as machine learning synthesis.

% State the Significance of Your Question -> make readers understand why they should read your report
The aim of this work is, then, to fill this gap, introducing a compact representation 

% What I did - State Your Claim | Objective | Hypothesis -> should clearly relate to the information gap
We aim to exploit redundancies in the constitution of a discrete sound wave to formulate an alternative, more compact and thus easier to analyse, representation. Implicit in this affirmation is the somewhat loose assumption that the wave considered is reasonably well behaved, in the sense that redundancies exist to be identified. 

To that end, we use the cycle as the basic building block of a signal and describe its changes both in the vertical axis, assumed to be due to the varying amplitude of the signal, and in the local horizontal axis, reflecting global frequency variations. 
...
\subsection{Identifying the Pseudo Cycles}

This work proposes the (pseudo) cycle as the building block of a discrete wave, assuming its waveform to be constant throughout the space of the digital signal; in other words, the adjective pseudo is used because the two other descriptions of the wave, namely, the envelope and the length of the pseudo cycles act into the pseudo cycles shape itself, modifying it in function of the time, during the lifetime of the signal.
...
Using the envelope detection algorithm presented in \textcite{2020TarjanoRobust} one can identify the frontiers, both positive and negative, of a digital signal: Those are respectively composed of the maxima and minima of each pseudo-cycle of a discrete signal. 

If we disconsider inaccuracies we could use either one of the frontiers indifferently to infer the exact position, waveform and amplitude of each pseudo-cycle and to reconstruct the original wave with those information. Consider, for example, the digital signal presented in figure \ref{fig:signalenvelope}
...
It can be seen that both frontiers provide approximately the same information about the location of each pseudo-cycle, albeit with a phase difference between them. 

However, as the algorithm proposed in \textcite{2020TarjanoRobust} is originally meant for envelope detection, maxima or minima that lies close to the already established frontiers are sometimes neglected, without significant impact in the shape of the envelope extracted.

This can be seen in the discrete representation of the sound of the key 33 of a grand piano in figure \ref{fig:frontiergaps}, as the black squares and diamonds: intuitively, one can see that some local maxima and minima were not detected, compromising the identification of all pseudo cycles. 

For our purposes, however, it is beneficial to accurately identify as many local maxima and minima, and the resulting pseudo cycles, as possible, as the impact on the shape of a pseudo cycle is significant.

We are going to assume that all the local extremes identified by the envelope detection algorithm are indeed correct; albeit not being strictily accurate, this approximation is very reasonable and simplifies further steps immensely, as we now need only to idenfity pseudo cycles with periods $ T $, that is, the difference between two adjacent maxima or minima, that are significantly bigger than the majority of the ones observed in the discrete wave, and identify their local maximum or minimum, respectively. 

To that end we are going to use the mode of the periods as the centrality measure, instead of the average. Similarly, we will use the mean absolute deviation in relation to the mode as a measure of the dispersion of the periods. 

In figure \ref{fig:modevsaverage} an except of the plot of the frequency of the errors is shown for both measures, for the case of a discrete wave representing the sound of the key 33 of a grand piano. 

The errors are the frequency of absolute difference between the centrality of the distribution of all combined periods, as defined by both the positive and negative frontiers, and their values.

In addition to the smaller mean absolute deviation, the errors when the mode is used are centered around zero, which is not the case for when the centrality measure used is the average.

\begin{figure}[ht!]
  \centering
     \includegraphics{images/02modevsaverage.pdf}
  \caption{Frequency of errors for the periods of a digital signal representing the sound of the key 33 of a grand piano when two different centrality measures are used: the mode of the periods and their average.}
  \label{fig:modevsaverage}
\end{figure}


The result of this procedure can be seen as the additional squares and diamonds in figure \ref{fig:frontiergaps}, representing new maxima and minima, respectively, found by iteratively reanalising and eventually splitting pulses whose period is superior to 2 mean absolute deviations from the mode of the periods distribution, both for the positive and the negative frontiers.

A new mode is then calculated from the new pulse distribution, as well as the mean absolute deviation; the procedure is repeated until no more new pseudo pulses are found or the mean absolute deviation is increased by the addition of the newfound extrema.

\begin{figure}[ht!]
  \centering
     \includegraphics{images/03frontiergaps.pdf}
...
Figure \ref{fig:plotXps} illustrates the behaviour of the extrema as inferred by both frontiers, before and after the refinement. It can be seen, particularly at the end of the wave that each inference is a little biased, both before and after the refinement process. To solve that, we can calculate an intermediate measure of both inferred values, by averaging compatible values of both frontiers.

In this way we have a balanced, unbiased estimative of the position of the pseudo pulses along the time that can be used to calculate the average waveform of the pseudo pulses in the signal of interest.

We won't, in general, have an accurate estimative - unless methods such as the FFT are used - of the relative phase of this signal, since the points marked by the average frontier wont, in general, concide with points of interest of the waveform.

In special cases such as sinusoids, this would be the case, as the average frontier would mark the zeros of each pseudo cycle. Nevertheless, our interest here is in periodicity, since the envetual extracted average waveform will have to be made tileable, rendering ineffective such a discussion. It will be shown below that the average waveform identified as described is indeed very close to tileable, and minimal further processing is required.

\begin{figure}[ht!]
  \centering
     \includegraphics{images/04plotXps.pdf}
  \caption{Frame of occurrence of the maxima and minima of a discrete signal representing the sound of the key 33 of a grand piano before and after the refinement process, as well as the average of both refined extrema.}
  \label{fig:plotXps}
\end{figure}

We could use a polinomial approximation, fitting an arbitrary direct polynomial to the values. Given the range of number, that procedure would bring numerical instabillity to our model, however. Instead, we chose to divide the approximation into a straight line and a polynomial approximation of the deviation from this line. In that way we are able to enforce the overall pitch of the wave, while avoiding computational instability.

\subsection{Pseudo Cycles evolution over time}
