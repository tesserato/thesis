...
'''###### Read wav file ######'''
W, fps = se.read_wav(f"images/00resources/{name}.wav")
W = W - np.average(W)
amp = np.max(np.abs(W))
W = W / amp
n = W.size
...
Xpc = np.genfromtxt(f"images/00resources/{name}.csv", delimiter=",")
...
fig.update_xaxes(showline=False, showgrid=False, zerolinewidth=1, zerolinecolor='black', range=[13405, 13925])
...
save_name = "./images/" + sys.argv[0].split('/')[-1].replace(".py", "_a.svg")
...
fig.show(config=dict({'scrollZoom': True}))
save_name = "./images/" + sys.argv[0].split('/')[-1].replace(".py", "_b.svg")
fig.write_image(save_name, width=650, height=300, engine="kaleido", format="svg")
print("saved:", save_name)
