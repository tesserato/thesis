...
sample based virtual instruments are the de facto standard for the emulation of real instruments. nevertheless little attention is given to the encoding of the samples used in those instruments; in fact, lossless containers like wav or proprietary formats are common, pushing the on disk size of those libraries to sizes exceeding the gigabytes.
% Motivation (Why do we care about the problem and the results?) 
This renders the most realistic digital instruments inacessible in many  but the most high end hardware, controllers, like electronic drumkits and controller keyboards. Even less robust personal computers might encounter problems.
% Problem | Objectives (What problem is the paper trying to solve?) (Scope)
We propose a lossy compression algorithm taylored specifically to highly uniform sounds, as those present in this instruments, due to the nature of the sounds, highly redudant, substantial improvements can be obtained, with minimal quality loss.
...
lends itself well to machine learning, as the sound can be represented by a evolving waveform over time (pseudo-cycles)
...
This transforms the instrument in a mere controller, robbing it of characteristics that provide the personality of the conventional instruments, such as timbre.
