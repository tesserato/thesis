...
#1.0e-159
scale = 1
pos_frontierX_scaled, pos_frontierY_scaled = pos_frontierX * scale, pos_frontierY
neg_frontierX_scaled, neg_frontierY_scaled = neg_frontierX * scale, neg_frontierY

while np.allclose(pos_frontierX_scaled / scale, pos_frontierX) and np.allclose(neg_frontierX_scaled / scale, neg_frontierX):
  print(scale)
  scale = scale * 10
  try:
    pos_frontierX_scaled, pos_frontierY_scaled = get_frontier(posX * scale, posY)
    neg_frontierX_scaled, neg_frontierY_scaled = get_frontier(negX * scale, negY)
  except:
    print(f"scale={scale}")
...
fig.update_xaxes(row=1, col=2, title_text="scaled $i$", showline=False, showgrid=False, zeroline=False)
