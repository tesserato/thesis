...
Envelope detection, also known as demodulation, is ubiquitous in analog and digital signal processing (DSP) \cite{2011CaetanoImproved}. Nevertheless, the literature in this area is fragmented \cite{2017LyonsDigital}, with most envelope detection techniques being designed to account for particular settings, in general involving narrowband signals with moderate noise content.

Those constraints exclude most physical signals, as is the case of recorded sound, for example; this problem arises in part due to the lack of a strict mathematical definition of a temporal envelope \cite{2014Yangnovel,2015Yangtheoretical,2019Jiaempirical}.
...
The temporal envelope also helps to impart emotion and identity to the human voice \cite{2018ZhuContributions} and music: Concert halls with envelope preserving characteristics are deemed more pleasant \cite{2011LokkiEngaging}.
...
Considering the standard graphic representation of a discrete wave, where the horizontal axis represents time and the vertical axis represents the amplitude, one can think of each sample as a point in a plane. Provided a coherent mapping from this space to the Cartesian plane is available, the original DSP problem can be transformed into the geometric problem of finding the shape of this set of points.

One direct way of defining the shape of a set of points in $ \mathbb{R}^2 $ is by using the concept of a convex hull, as defined in \cite{2008BergComputational}. The convex hull of a set of points $ S $ in two-dimensional space is the smallest convex subset of the plane that contains all the points in $ S $. Convex hulls are unique, in the sense that a given set of points define one and only one convex hull.
...
Fig. \ref{fig:AlphaShape} illustrates an alpha shape, for $ \alpha=10 $, of a set of points. Note that two points of the set lie at the circle illustrated, but the open disc defined by it contains no points of the set. An analogous construction is possible for every point at the frontier. 

Conversely, any circle of radius $ \alpha=10 $ that contains at least one of the interior defines an open disc that also contains at least another point of the set P.
...
Once a radius $ \alpha $ is defined, alpha shapes exhibit the uniqueness property: for a given set of points and a given radius, only one alpha shape exists. When the radius $ \alpha $ of the circle tends to infinity, alpha shapes, also known as concave hulls, tend to the convex hulls.

Consider, for example, the artificially generated discrete wave in Fig. \ref{fig:Hulls} below, where the links between consecutive samples were de-emphasized to highlight the “set of points” nature of a signal: two alpha shapes, with different values for $ \alpha $, and the convex hull are illustrated. As $ \alpha $ increases, the contour of the set of points becomes smoother, and closer to the convex hull.
...
The alpha shapes theory is used in areas such as detection of features in images \cite{2016VarytimidisAlpha}, reconstruction of surfaces from a cloud of points \cite{2015WuAutomated} and spectroscopy \cite{2019XuModeling}. 
...
We start with a brief background of envelope detection in Section \ref{sec:envelope} and define, as precisely as possible, what constitutes a temporal envelope. 
...
Section \ref{sec:quality} discusses the difficulties in assessing the quality of an envelope, proposing qualitative and quantitative approaches, and demonstrates that the method presented conforms to four requirements for a physically plausible envelope proposed in \cite{1996Loughlinamplitude}.
 
In Section \ref{sec:comparison} we compare the presented algorithm with traditional envelope detection methods in terms of accuracy and time and present, in Section \ref{sec:extensions}, potential applications of the algorithm, some of which might not be immediately obvious.
...
A Python module can be installed from the Python Package Index (PyPI) with the command “pip install signal-envelope”. Besides, those working under Windows 64-bit can benefit from a specialized dynamic library coded in C++ that is automatically used within this module, and can be separately obtained via the release section in the repository.
...
While there is no general formal definition for the envelope of broadband signals \cite{2014Yangnovel,2015Yangtheoretical,2019Jiaempirical}, the consensus is that an envelope is a smooth curve situated as close as possible to the signal \cite{2019Jiaempirical}.

In this context, the most mathematically sound definition of an envelope, as introduced by Gabor in 1946 \cite{2007HahnHistory}, involves representing the underlying wave as an analytic signal. In his work, \cite{1946GaborTheory} applies the then relatively new mathematical machinery of quantum mechanics to unify the time and frequency-domain representations of a wave, showing how the Hilbert transform could be applied to a real signal in order to obtain an equivalent complex signal, later known as the analytic signal.

This analytic signal has the form $ A(t) = S(t) + \mathbb{H}(S(t)) \ \text{i} $ \cite{2016HePraat} where $ S(t) $ is the original real signal. $ \mathbb{H}(S(t)) $, the Hilbert transform of the original signal, becomes the imaginary part of the analytic signal; the envelope of a signal thus represented can be straightforwardly obtained by the computation of the complex modulus of $ \mathbb{H}(S(t)) $. This representation is common in classical AM-FM literature.
...
Fig. \ref{fig:AnalyticSignal} exemplifies the Hilbert envelope, for a pure sinusoid modulated by a polynomial, illustrating the difference in the smoothness of the envelope in the presence and absence of Gaussian white noise. It is readily noticeable from Fig. \ref{fig:AnalyticSignal} that, as soon as noise is introduced in the original signal, it is directly reflected in the envelope.

\begin{figure}[ht!]
  \centering
    \includegraphics{images/01AnalyticSignal.pdf}
  \caption{Envelope of a pure sinusoid with a local frequency of 20 cycles, modulated by a polynomial of degree 3, as obtained by the Hilbert Transform approach. In the first half of the samples, the sinusoid is free of noise, while in the second half Gaussian noise, with a standard deviation of $ 1/10 $ of the maximum amplitude of the wave, was added to the base signal.}
  \label{fig:AnalyticSignal}
\end{figure}

For real-world signals, such as an alto singer sustaining a steady note as shown in Fig. \ref{fig:AnalyticSignalAlto}, the Hilbert transform alone provides a poor envelope, since it retains most of the frequencies of the underlying wave. With further processing, however, such as filtering, a smoother output can be obtained.

\begin{figure}[ht!]
  \centering
    \includegraphics{images/01AnalyticSignalAlto.pdf}
  \caption{A section of the representation of an alto singer sustaining a steady note comparing the envelopes obtained by the Hilbert transform with and without further filtering.}
...
We address this problem by assuming $ C(t) $ to be normalized between $ \{-1, 1\}$ or, in other words $ |\max(C(t))| = 1 $. This assumption does not cause any loss of generality since, given an arbitrary $ C(t) $, we can obtain its normalization by dividing the function by its absolute global maximum, that is, $ \hat{C}(t) = C(t) / \max(|C(t)|) $, provided that $ \max(|C(t)|) \ne 0 $.

In this work we are concerned with the discrete version of the envelope detection problem: given a finite digital wave represented by the vector $ \mathbf{w} \in \mathbb{R}^n $, obtaining the temporal envelope $ \mathbf{e} $ of $ \mathbf{w} $. 
...
The $ \odot $ operator in Equation \ref{eq:Envelope} stands for the Hadamard product, denoting element-wise multiplication of two vectors. Fig. \ref{fig:Envelope} provides an example of the vectors just defined: \textbf{w} is obtained by multiplying the normalized carrier wave \textbf{c} using the envelope \textbf{e}.
...
While the exact relation between $ n $, the number of samples of the original signal, and $ m $, the cardinality of the set of points, is dependent on factors such as the frequency and sampling rate of the original wave, the inequality $ m \le n $ holds for all discrete signals. In practice, $ m \ll n $ for most real-world signals.
...
To achieve that, we divide the ordinate of each point by the average of all ordinates, effectively canceling the unit of the vertical components of the points. If we proceeded similarly concerning the horizontal components,
...
Algorithms such as the convex hull and the alpha shapes solve that problem by discriminating which points of a set are part of its frontier.
...
Alternatively, one can picture a circle of fixed radius being rolled above the set P, and marking the points it touches as part of the envelope.
...
The two general approaches are the derivation of direct methods that use characteristics of the discrete curve to calculate the curvature, or the use of the curvature of a smooth, continuous curve fitted to the original discrete curve \cite{2001CoeurjollyDiscrete}. Since we are dealing with a potentially large set of points, it is important for this measure to act locally, to assure $ O(n) $ complexity, as is generally not the case for approaches that involve curve fitting.

Concerning direct methods \cite{2014CarrollSurvey}, derive three such definitions based on the approximation of a circle by an inscribed, centered, and circumscribed polygon. In the context of three-dimensional meshes, \cite{2016VasaMultivariate} evaluate a range of existing estimators from a multivariate point of view.
...
We formalize the set V of vectors in Equation \ref{eq:Vectors} below.
...
One of those edges, in the form of the vector $ v_k \in \text{V} $, is singled out in Fig. \ref{fig:DiscreteCurvature}. $ v_k $ represents the net displacement of the envelope from the extrema $ P_{j} = (x_j, y_j) $ to $ P_{j+1} = (x_{j+1}, y_{j+1}) $. 

We assume from $ v_k $ that, near the vector, the envelope defines a vertical displacement of $ v_{k,y} $ in the horizontal distance of $ v_{k,x} $; since we don't yet know which points of P are part of the envelope, we can't be certain that the envelope passes through $ P_{j-1} $ before reaching $ P_{j} $, or even if it reaches $ P_{j} $. 

Nevertheless, assuming it passes through $ P_{j} $, and is limited to the diplacement observed, the envelope might have passed anywhere in the line segment from points $ (x_j - (x_{j+1} - x_j), y_j - (y_{j+1} - y_j)) $ to $ (x_j - (x_{j+1} - x_j), y_j + (y_{j+1} - y_j)) $, the bold line segment before $ v_k $ in Fig. \ref{fig:DiscreteCurvature}.
...
The rationale then is to find, for each vector $ v_k $, the radius $ r_k $ of the equivalent circle whose unit tangent performs the same change in direction, in the same horizontal distance, as illustrated in Fig. \ref{fig:DiscreteCurvature}. The average curvature of P will be obtained by the average of all radii $ r_k $.
...
The radius $ r_k $ of the circle equivalent to each $ v_k $ is defined in Equation \ref{eq:Radius}. This equation has the desirable property that $ v_{k,y} \to 0 \implies v_{k,y} \to \infty $, that is, the radius tends to infinity when no vertical oscillation occurs, effectively turning the circle into a line. 
...
We need to construct an algorithm, using the radius just obtained, to identify the points that belong to the envelope. Generally, alpha shapes based algorithms resort to the construction of the Delaunay triangulation of the set of points, which is later filtered to contain only the outer edges. We can adopt a more straightforward approach, since our points have a sequential structure. 

Let $ x_o, y_o \in \mathbb{R}, \quad 0 \le y_o $ be the coordinates of the center of a circle with radius $ r $, that can be placed anywhere in the upper plane of the coordinate system;

Let $ P_a, P_b \in \text{P}, \quad P_a \ne P_b $ be two different points of the set P;

For all circles of center $ x_o, y_o $ and radius $ r $ that can be constructed to pass through any two different points of P, that is, all circles such that $ (P_{a,x} - x_o)^2 + (P_{a,y} - y_o)^2 = (P_{b,x} - x_o)^2 + (P_{b,y} - y_o)^2 = r^2 $, $ P_a $ and $ P_b $ will be part of the set $ E $ of the points belonging to the envelope if and only if exist one such circle that defines a disc that does not contain any other point of P. Summarizing this, we have Eq. \ref{eq:InEnvelope}:
...
The set E obtained via the algorithm \ref{FindEnvelope} is a subset of P, and thus also a set of points in the Euclidean space. 
