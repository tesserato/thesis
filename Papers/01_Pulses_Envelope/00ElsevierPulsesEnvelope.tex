...
Envelope detection, also known as demodulation, is ubiquitous in both analog and digital signal processing (DSP) \cite{2011CaetanoImproved}. Nevertheless, the literature in this area is fragmented \cite{2017LyonsDigital}, with most envelope detection techniques being designed to account for very specific settings, in general involving narrowband signals with moderate noise content.
...
Fig. \ref{fig:AlphaShape} illustrates an alpha shape, for $ \alpha=10 $, of a set of points. Note that two points of the set lie at the circle illustrated, but the open disc defined by it contains no points of the set. An analogous construction is possible for every point at the frontier. For all the interior points, it is impossible to construct a circle of radius 10 that contains one or more of them without defining a disc containing at least one point of the set. 
...
In Section \ref{sec:representation} we explore some simplifications to the task of detecting the envelope of a signal that arise from the envelope definition adopted in this work. 
...
Section \ref{sec:quality} discusses the difficulties in the assessment of the quality of an envelope, proposing qualitative and quantitative approaches, and demonstrates that the method presented conforms to four requirements for a physically plausible envelope proposed in \cite{1996Loughlinamplitude}.
...
\begin{figure}[ht!]
  ...
  \caption{A section of the representation of an alto singer sustaining a steady note and a comparison of the envelopes obtained by the Hilbert transform with and without further filtering.}
...
\begin{split}
  ...
  \mathbf{e} & = (e_0, e_1, \cdots, e_{n-1}), e_i \ge 0 \ \forall \ i, \ 0 \le i \le n-1 \\  
  \mathbf{c} & = (c_0, c_1, \cdots, c_{n-1}), |c_i| \le 1 \ \forall \ i, \ 0 \le i \le n-1, |\max(\mathbf{c})| =1 \\
...
To achieve that, we divide the ordinate of each point by the average of all ordinates, effectively canceling the unit of the vertical components of the points. If we proceeded similarly in relation to the horizontal components,
...
Having established a transformation from the original space of the two-dimensional representation of a discrete wave to the Cartesian coordinate system, we are now in a position to apply the geometric concept of the shape of a set of points.
...
The rationale then is to find, for each vector $ v_k $, the radius $ r_k $ of the equivalent circle whose tangent has the same change in direction, in the same horizontal distance, as the vector and the horizontal direction, as shown in Fig. \ref{fig:DiscreteCurvature}. The average curvature of P will then be obtained by the average of all radii $ r_k $.
...
We need now to construct an algorithm, using the radius just obtained, to identify the points that belong to the envelope. Generally, alpha shapes based algorithms resort to the construction of the Delaunay triangulation of the set of points, which is later filtered to contain only the outer edges. We can adopt a more straightforward method, as our points have a sequential structure. 
...
In order to obtain a numeric indicator of the quality of an envelope, we construct a reference envelope, that trades off applicability in favour of mathematical formality.
...
In practical applications, one seldom has access to a family of curves, instead being interested in obtaining the envelope of a single signal. Nevertheless, an approximation can be obtained, by shifting the original signal along its initial position in the horizontal axis by half of its period.
...
The envelope obtained is not smooth, being composed of a series of horizontal segments, as can be seen in \ref{fig:RefEnvelope}, besides being computationally expensive to calculate. However, arising from a mathematically sound basis, it can serve as a reference to infer the quality of more empirical methods.
...
The frontiers were satisfactorily detected in the case of harmonic and inharmonic sounds, and are robust in relation to the number of samples and the frequencies of the waves. It is worth noting that the positive and negative frontiers are generally smoother than the unified envelope and seems to conform better to the contours of the underlying discrete signal.
 
\subsection{Simplified spectral representation} 
 
The temporal envelope adds complexity to the spectral representation of a wave \cite{2019TarjanoNeuro}, and an accurate representation of the envelope, while describing the evolution of the instantaneous amplitude of a signal in time, would also simplify further spectral analysis. 
...
The vertical lines mark the indices of the samples that belong to the positive frontier of the original signal, and the section between each vertical line isolates a pseudo cycle of the wave. Note that, although two local maxima exist in each pseudo cycle, only the largest was marked by the algorithm.
