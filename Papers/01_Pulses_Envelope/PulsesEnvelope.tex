...
When the general shape of a set of points present prominent concavities, the convex hull overestimates the boundaries. To address this limitation \textcite{1983Edelsbrunnershape} introduced the concept of alpha shapes, also known as concave hull: a mathematically well-defined generalization of the convex hull of a finite set of points, closely related to the Delaunay triangulation and Voronoi diagrams of those points. 
...
We are interested in finding a change of basis formula from the original basis to the standard basis $ \mathbf{s} = \{ \mathbf{s}_x, \mathbf{s}_y \} = \{ (1,0),(0,1) \}$ of this vector space, making the geometric representation consistent.
...
Considering $P_{\mathbf{b}}$ as a point of the set P represented by the original basis and $P_{\mathbf{s}}$ the same point as represented with the standard basis, we can formalize this change of basis as in \ref{eq:Basis}:
...
A formal definition of alpha-shapes is given at \textcite{1983Edelsbrunnershape}. In the context of this work, gaining the intuition of the algorithm is more important, and thus the explanation of the alpha-shapes algorithm presented in \textcite{1994EdelsbrunnerThree} can be more insightful.
...
In an effort to formalize the assessment of the quality of amplitude and frequency modulations of a signal, \textcite{1996Loughlinamplitude} proposed some theoretical conditions necessary, but not sufficient, to ensure the physical plausibility of an envelope. 
...
To see that this condition holds for the proposed method, consider a discrete signal $ \mathbf{w} = (w_0, w_1, \cdots, w_{n-1}) $, where all the absolute values $ |P_j| $ of its local extrema are greater than one.
