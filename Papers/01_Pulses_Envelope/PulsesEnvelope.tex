...
Translating the explanation of the alpha-shapes algorithm presented in \textcite{1994EdelsbrunnerThree} to our context, we can say that the points that are part of the envelope are those that can be touched by a circle of a given radius $ r $, that must not contain any point of the signal, coming from $ y \to \infty $ towards the Cartesian representation of the signal.

% TODO formal definition of alpha shapes, maybe image

Intuitively, one can picture a circle of fixed radius being rolled above the signal, and marking the points it touches as envelope points.
...
We thus proceed to define a discrete curvature measure over the edges of a discrete curve. To that end we are going to apply the definition of smooth curvature as the rate of change of the unit tangent to a curve. This definition is convenient, since it is equivalent to that of the osculating circle \parencite{2016VasaMesh}.

One of those edges, a vector $ v_k \in \text{V} $, is singled out in Fig. \ref{fig:DiscreteCurvature}. $ v_k $ represents the net displacement of the envelope from the extrema $ P_{j} $ to $ P_{j+1} $. Since we dont yet know which points of P are part of the envelope, instead of assuming the trajectory before $ P_{j} $ passes through  
$ P_{j-1} $, it is more prudent to assume that it can have passed anywhere in the line segment from 
% TODO
That is because all we can assume from $ v_k $ is that, in this current region of the envelope, it is able to a vertical displacement of $ v_{k,y} $ in the same time as it performs a horizontal displacement of $ v_{k,x} $. In average, we can consider the envelope entering from the horizontal direction.
...
The radius of the circle equivalent to each $ v_k $ is defined in Eq. \ref{eq:Radius}. This equation has the desirable property that $ v_{k,y} \to 0 \implies v_{k,y} \to \infty $, that is, the radius tends to infinity when no vertical oscillation occurs, effectively turning the circle into a line. 
% It is also directly proportional to $ v_{k,x} $, making the radius larger the more the points are horizontally far from each other.

\begin{equation} \label{eq:Radius}
  r_k = \frac{v_{k,x} \sqrt{v_{k,x}^2 + v_{k,y}^2}}{ v_{k,y}}
\end{equation}

For calculation purposes, however, this can cause a problem, as a zero could make its way into the denominator of a fraction; in the actual algorithm, the quantity calculated for each vector is the curvature itself, that is, the inverse of the radius. In this way, the problem is avoided, since $ v_{k,y} $ is guaranteed to be greater than zero. The average of curvatures is then inverted in order to obtain the average radius, as illustrated in Eq. \ref{eq:Curvature}.

\begin{equation} \label{eq:Curvature}
r = \left( \frac{\sum\limits_{k=0}^{m-2} \left( \frac{ v_{k,y}}{v_{k,x} \sqrt{v_{k,x}^2 + v_{k,y}^2}} \right)}{m-2} \right)^{-1}
\end{equation}
...
The procedure is formalized in the algorithm \ref{FindEnvelope}. We don't provide similar algorithmic descriptions of the preceding steps since the Python implementation available at \textcite{2020TarjanoEnvelope} can easily serve as pseudocode.
