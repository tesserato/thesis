...
Geometrically, we can think of these points as points in a vector space described by a non-standard basis vector $ \mathbf{b} = \{ \mathbf{b}_x, \mathbf{b}_y \}$ with $ \mathbf{b}_x = (a, 0), \mathbf{b}_y = (0, i) $ where $ a $ is an amplitude unit such as decibel, a measure of the instantaneous sound pressure, volt or even a fraction of a fixed maximum value. $ i $ represents the index of the sample, and is linked to time by the aforementioned relation $ i = t \ \text{fps} $.

We are interested in finding a change of basis formula from the original basis to the standard basis $ \mathbf{e} = \{ \mathbf{e}_x, \mathbf{e}_y \} = \{ (1,0),(0,1) \}$ of this vector space, making the geometric representation consistent.
...
the direct link between P and \textbf{w} that arises from the fact that the abscissas of the points in P are indices $ i $ of \textbf{w} would be lost.
...
Considering $P_{\mathbf{b}}$ as a point of the set P represented by the original basis and $P_{\mathbf{e}}$ the same point as represented with the standard basis, we can formalize this change of basis as in \ref{eq:Basis}:

\begin{subequations}  \label{eq:Basis}
\begin{align}
P_{\mathbf{b}} &= (x_1, x_2) \\
\beta &= \frac{\left( \frac{i_{m-1} - i_m}{m-1} \right)}{\left( \frac{\sum_{j=0}^{m-1} \lvert w_j \lvert}{m} \right)}  \\
P_{\mathbf{e}} &= (x_1, \beta x_2) \quad \forall \  P \in \text{P} \\ 
\end{align}
\end{subequations}
...
\section{Extracting the envelope}
...
\begin{subequations} \label{eq:Req1}
\begin{align}
...
If the original signal is bounded, that is $ \max (|w_0|, |w_1|, \cdots, |w_{n-1}|) = b \in \mathbb{R} $, not only our envelope is bounded, as required, but is guaranteed to have the same bound $ b $.
...
The fourth and last condition states that if a wave is multiplied by a constant, the envelope should also be multiplied by the same constant. 

Recall from section \ref{mapping} that, before the extraction of the envelope, the original points undergo a change of basis to lie in the Cartesian coordinate system. After the extraction, the inverse operation is performed on the points of the Envelope.
...
Only the present algorithm and the one based on the Hilbert transform comply with the third condition proposed by \textcite{1996Loughlinamplitude}, that states that the envelope of a periodic wave should be a straight line.
...
We can see from the table \ref{table:Error_comparison} that the method presented here consistently outperforms the traditional approaches for all waves tested, yielding an error 
33\% smaller, % TODO
on average, than the other methods. It is also worth noting that the errors of the traditional methods are very similar between them, in agreement with the visualizations.
...
The temporal envelope adds complexity to the spectral representation of a wave \parencite{2019TarjanoNeuro}, and an accurate description of the envelope, while describing the evolution of the instantaneous amplitude of a signal in time, would also simplify further spectral analysis. 
