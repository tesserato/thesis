...
\section{Characterization of the temporal envelope}
...
\begin{figure}[ht!]
  ...
  \caption{Envelope of a pure sinusoid modulated by a polynomial of degree 3, as obtained by the Hilbert Transform approach. In the first half, the sinusoid is free of noise, while in the second half white gaussian noise with a standard deviation of $ 1/10 $ of the maximum amplitude of the wave was added to the base signal.}
...
We address this problem by assuming $ C(t) $ normalized between $ \{-1, 1\}$. This assumption doesn't cause any loss of generality since, given an arbitrary $ C(t) $, we can obtain its normalization by dividing the function by its absolute global maximum, that is $ \hat{C}(t) = C(t) / \max(|C(t)|) $ provided that $ \max(|C(t)|) != 0 $.

Also, in this work we are concerned with the discrete version of this problem: given a finite digital wave represented by the vector $ W \in \mathbb{R}^n $, obtaining the temporal envelope of $ W $. The preceding definitions can be translated to a discrete setting assuming that the discrete quantities arise from observing the continuous ones at regular time intervals. In that case, the equality $ i = t \ \mathit{fps} $ can be used, where $ i $ is the index of each observation, $ t $ stands for the time in seconds, and $ \mathit{fps} $ is the frame rate, or the number of observations made in the period of a second. 

% <|<|<|<|<| TODO We can thus define:
...
% Overview of methods
 ...
 Since $ W $ is a wave, we expect it to be quasi-periodic, as well as uniformly sampled. 

Instrumental to the method here presented is the definition of a pulse $ P \subset W $: each time $ W $ crosses the horizontal axis, the beginning of a pulse is defined, with the next crossing defining its end. 

This definition is in line with the one presented in \textcite{national1996telecommunications}, where a pulse is defined as
a rapid change in the amplitude of a signal, followed by a fast return to the baseline value.

With a pulse thus defined, we can interpret a wave as composed of a succession of alternating positive and negative pulses.


As such, some structures can be defined, as illustrated in figure \ref{fig:Pulses}.

$ W = \{w_0, w_1, \cdots, w_{n-1}\} $

% <|<|<|<|<|

Half of a pseudo cycle, then, can be defined as the maximum set of adjacent samples with the same sign. Two adjacent half pseudo cycles form a full pseudo cycle, with the whole wave being formed by groups of adjacent pseudo cycles. The wave in figure \ref{fig:Pulses} if formed by 100 samples, that can be divided in 7 half pseudo cycles, of which two - the leftmost and the rightmost - are malformed, since they are not completely observable in the data provided by $ W $. 

With the remaining half pseudo cycles, two full pseudo cycles can be formed. Given the quasi-periodicity of the digital wave $ W $, the order in which two adjacent half pseudo cycles are joined to form a full pseudo cycle can be chosen arbitrarily, provided consistency is maintained; in figure \ref{fig:Pulses}, a positive half pseudo cycle is followed by the adjacent negative one, forming a pseudo cycle.

\begin{figure}[ht!]
  \centering
    \includegraphics{images/01Pulses.pdf}
  \caption{Example of a discrete wave. The black circles are the individual samples of the signal, representing the intensity of the wave at regular time intervals. The grey areas are the half pseudo cycles. The dashed lines mark the divisions between adjacent pseudo cycles.}
  \label{fig:Pulses}
\end{figure}

From figure \ref{fig:Pulses} and keeping in mind the fact that the variation of the temporal envelope is much slower than the temporal fine structure variation, it is reasonable to suppose that the envelope would ideally pass through the point of maximum amplitude of each half pseudo cycle, following the least sinuous trajectory possible, while still tangencing each half pseudo cycle.
...
The best representation one can hope to get of the original wave is, then, the digital wave itself. It is sensible, thus, to characterize the envelope of the wave in terms of a subset of the exact samples that comprise its digital representation.
