...
% TODO vague definition of envelope - smoothness and slow varying
...
The $ \odot $ operator in \ref{eq:Envelope} stands for the Hadamard product, denoting element-wise multiplication of two vectors. Fig. \ref{fig:Envelope} provides an example of the vectors just defined: \textbf{w} is obtained by modulating the normalized carrier wave \textbf{c} using the envelope \textbf{e}.

Note from Fig. \ref{fig:Envelope} that, at some local maxima, the envelope is well-defined, being in fact equal to the value of the wave $ w_i $. $ c_i = 1 $ at those points, rendering $ e_i = 1 w_i $. 
...
In this new coordinate system we have a basis vector $ \mathbf{b}^\prime = \{ \mathbf{b}^\prime_x, \mathbf{b}^\prime_y \}$ where $ \mathbf{b}^\prime_x = (i, 0) $ and $\mathbf{b}^\prime_y = (0, i) $, related to the original basis vector by Eq. \ref{eq:Basis}:
...
The effect of this normalization can be seen in the points shown in Fig.\ref{fig:Vectors}, shown in scale in both coordinate systems. 
...
To employ the alpha shapes algorithm in this set of points, however, the radius $ r $ of such circle must be estimated, and for that a measure of discrete curvature is needed.

The set V of the vectors from each point of P to the next is also shown in the picture, and it will be used to estimate the average curvature of P. We define this set in Eq. \ref{eq:Vectors} below. % TODO
...
There are no agreed-upon objective metrics to assess the quality of an envelope. Recalling Fig. \ref{fig:Envelope}, however, and the accompanying discussion, an intuitive way is to consider the carrier wave \textbf{c}, and how well it is bounded by the interval $ \{-1, 1\} $.
...
These characteristics are in stark contrast with those of the sang voice in Fig. \ref{fig:singer_voice}, where a recording of an alto singer is shown; there, the wave is almost periodic, with a stable waveform.
...
Fig. \ref{fig:Bend} shows the envelope in the case of a bend performed in an eletric guitar. The variations in frequency in the signal didn't affect the envelope extracted.
...
In Fig. \ref{fig:Comparison_soprano} all envelopes exhibit a similar movement along the samples, but the ones generated by traditional algorithms are undershooting the original wave.
...
\section{Superior and inferior envelopes - Frontiers} 
